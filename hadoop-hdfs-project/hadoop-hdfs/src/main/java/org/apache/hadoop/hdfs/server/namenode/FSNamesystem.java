...
public class FSNamesystem implements Namesystem, FSNamesystemMBean,
    ...
    void startCommonServices(Configuration conf, HAContext haContext) throws IOException {
        ...
        try {
            //NameNode资源检查，通过core-site.xml，hdfs-site.xml两个文件就知道元数据在哪里
            //需要检查三个目录:  editlog
            //两个目录:存储fdfsimage目录文件，存储editlog的目录。一般默认情况下这两个目录是同一个目录
            //Journlanode中也有存放元数据的目录
            //里面获取到了要检查的目录
            ...
            //HDFS 安全模式的检查(据说这里高频面试)
            /**
             * 1)在操作hadoop经常进入安全模式----为什么进入安全模式
             */
            setBlockTotal();
            //启动重要的服务
    ...
    public class SafeModeInfo {
         ...
         * 是否进入安全模式，
         * 进入安全模式3条件：
         * 1)namenode启动之后datanode也要启动，且上报自己的block，
         * 如果datanode的block数量小于nomenode的最小节点信息，进入安全模式
         * 2)datanode需要和namenode进行心跳，如果小于，安全
         * 3)磁盘空间是否充足
         */
        private boolean needEnter() {
            //namenode启动起来之后，datanode也要启动起来，且要上报自己的block信息
            //如果datanode上报的节点信息小于最小阈值的block，进入安全模式
            //且datanode还需要与namenode进行心跳，一个集群里面，
            //如果存货的dataNode个数少于几个节点，集群处于安全模式。
    ...
    public void setBlockTotal() {
        ...
        if (safeMode == null)
            return;
        //设置安全模式
        //getCompleteBlocksTotal获取所有正常使用的block个数
        //计算是否达到消除安全模式
